...
def render_report(bundle: dict[str, Any]) -> str:
    ...
    return f"""<!doctype html><html><head><meta charset="utf-8"><title>OceanTrace case {_h(case['id'])}</title>
<style>
body{{font-family:Inter,Segoe UI,Arial,sans-serif;margin:32px;color:#0b1220;line-height:1.45}}
h1{{margin:0 0 4px}} h2{{margin-top:28px;border-bottom:2px solid #0ea5e9;padding-bottom:4px}}
table{{border-collapse:collapse;width:100%;font-size:13px}} th,td{{border:1px solid #cbd5e1;padding:6px;vertical-align:top;text-align:left}}
th{{background:#e2e8f0}} .badge{{display:inline-block;padding:2px 8px;border-radius:10px;background:#0ea5e9;color:#fff;font-size:11px;font-weight:700}}
.mode{{background:#f59e0b}} .warn{{background:#fef3c7;border-left:4px solid #f59e0b;padding:8px}} .lim li,.ev li{{font-size:12px}}
.disc{{background:#f1f5f9;border:1px solid #cbd5e1;padding:10px;font-size:12px}} code{{font-size:11px;word-break:break-all}}
@media print{{body{{margin:12mm}}}}
</style></head><body>
<h1>OceanTrace AI — Investigation Case Report</h1>
<p><b>{_h(case['name'])}</b> · {_h(case['id'])} · status {_h(case['status'])} · <span class="badge mode">DATA MODE: {_h(mode)}</span> · exported {_h(bundle.get('exported_at_utc'))}</p>
...
<p><small>Config snapshot hash: {_h(case.get('config_hash'))}. Generated by OceanTrace AI v0.3 — output is investigation support, not a legal determination.</small></p>
</body></html>"""
